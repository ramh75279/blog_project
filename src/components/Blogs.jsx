...
function Blogs() {
    ...
    useEffect(() => {
        ...
        auth.onAuthStateChanged(function (user) {
            if (user) {
                if (user.uid === "cZYVNIULl4XDCcIxkCQ4IZqKWd32") {
                    setadmin(true)
                }
                else {
                    setadmin(false)
        ...
        axios.get("https://blog-project-backend-3-mnlx.onrender.com/api/blogs").then((res) => {
    ...
    const handleLike = async (blog_id) => {
        try {
            const response = await axios.patch(`https://blog-project-backend-3-mnlx.onrender.com/api/blogs/like/${blog_id}`);
    ...
    const handleNewBlogSubmit = (event) => {
        ...
        const likes = 0
        axios.post("https://blog-project-backend-3-mnlx.onrender.com/api/blogs", { newTitle, date, newContent, likes }).then((res) => {
            console.log(res.data)
